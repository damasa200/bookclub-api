...
class UserBookController {
  async create(req, res){
    try {
      ...
      if (!user){
       ...
       if(!book) {
        return res.status(404).json ({error: "Livro não encontrato"})
       } 
  ...
  async delete (req, res){
    try {
      ...
      if(!userbook){
        return res.status(400).json({error: " Livro não encontrato."});
      }

      if(userbook.user_id !== req.userId) {
        return res.status(401).json({error:"Registro não pertence ao usuário"});
      }
